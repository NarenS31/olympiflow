...
destroys the one signal that would have revealed the evidence was fabricated.
...
precision, recall, $F_1$ and hallucination $(1-\text{precision})$ follow. Every
figure we report is \emph{macro}-averaged --- the mean over scenarios of that
scenario's own value --- so $F_1$ is the mean of per-scenario $F_1$ and, $F_1$ being
concave in precision and recall, sits below the harmonic mean of the aggregate
precision and recall shown beside it. The resolver scores $29/30 = 96.7\%$ on hand-labeled cases --- a \textbf{single-labeler
...
we measure it three ways. Splitting each target's 24 windows into two disjoint
stratum-balanced halves and re-deriving the top-8 from each gives a \emph{pooled}
edge-set Jaccard --- top-8 identity across the full node set, not restricted to any
adjacency or reachability stratum --- of $0.2256$ over the 200 free-flow targets
present in both halves and $0.1399$ over the 28 congested ones, against a
two-independent-random-draws reference of $0.0165$ and
$0.0182$. Computed instead \emph{per target and then averaged} --- a
different statistic, hence the spread --- split-half agreement over 40 targets is
$0.2323\pm0.1461$ at the committed sparsity coefficient, $0.2385\pm0.1739$ at
$3\lambda$ and $0.2287\pm0.1831$ at $10\lambda$: raising sparsity does not
help.\footnote{Pooled figures: run \texttt{20260824T235016Z\_\_influence\_\allowbreak
graph\_solves}, \texttt{metrics.json}, \texttt{per\_regime.*.stability.\allowbreak
jaccard\_all\_edges} --- a Jaccard over the union of every target's top-8
source--target pairs, hence no standard deviation; the adjacency-restricted variants
are separate fields and are not used. Per-target figures and the entropy results
below: run \texttt{20260825T201433Z\_\_grounding\_\allowbreak without\_information},
\texttt{part4\_metrics.json}.} Re-running the explainer at a different random seed on 10 of those
targets recovers only $0.3480$, $0.3149$ and $0.3015$ of the top-8. Even the
arithmetic is load-bearing: solving the same target and window at the same seed on
CPU versus MPS agrees on importance \emph{magnitudes} (Pearson $0.9193$) while
agreeing on top-8 \emph{identity} only $0.3991$ of the
time.\footnote{Run \texttt{20260824T235643Z\_\_device\_\allowbreak divergence},
...
\begin{figure}[t]
\centering
\includegraphics[width=0.78\linewidth]{fig_loop.pdf}
...
A deterministic checker's facet coverage is settled by construction rather than
measurement. \emph{Surface-form sensitivity}, \emph{positional bias},
\emph{sycophancy} and \emph{inter-judge inconsistency} are removed outright --- there
is no judge model to exhibit them and the same input yields the same verdict every
run --- as is \emph{verbosity bias}, since the score depends on the citation array,
not answer length. Against that, three facets are untouched. \emph{Criteria drift} is not addressed: the
evaluation field is self-declared by the project that built the system, with no
external specification. \emph{Reasoning-chain validity} is not addressed: the judge
checks which locations are cited, never whether the causal story about them is
correct, so a reversed edge scores clean. And \emph{construct validity} is what this
paper puts in question --- a judge scoring random evidence identically to real
evidence measures citation compliance, not grounding.
...
a measured reliability. \textbf{(f)} The supporting measurements carry small samples:
seed-repeat 10 targets, device comparison 3 cases, SHAP 28 scenarios, congested
split-half 28 targets. They are indicative; the central claim rests on
Table~\ref{tab:main}. \textbf{(g)} The decision
...
irrelevant content is the evidence the model is told to cite.

The stability results sit on existing work on GNN-explainer evaluation:
\citet{sanchez2020evaluating} benchmark attribution against computable ground truth
on synthetic graphs and \citet{agarwal2023evaluating} supply generators and a library
for the same purpose, while \citet{faber2021ground} show that comparing explanations
to ground truth is itself unsound when the GNN does not use the ground-truth edges,
separating responsible, causing and explanatory motifs. Our setting has no ground
truth at all --- which is what makes a randomised-evidence control necessary rather
than optional --- and our mismatch lies one step further downstream, between an
explanation and the judge that consumes it; what we add is the consequence, that an
unstable, near-flat attribution is indistinguishable from noise to that judge and to
the agent built on it.
To our knowledge this combination --- \emph{GNN-explanation artifact}
as cited source, \emph{deterministic} verifier, no gold reference, and a
\emph{correction loop} measured against noise --- has not been evaluated before.\footnote{C\_RICH (fuller context) and D\_CONTRA (contradictory context) ran on
...
Split-half J, \emph{pooled} (200 / 28 targets) & 0.2256 / 0.1399; ref 0.0165 / 0.0182 & same file, \texttt{per\_regime.*.stability.\allowbreak jaccard\_all\_edges} \\
SHAP overlap and scores & J 0.0221; $F_1$ 0.7423 / 0.6918; P 0.9821 & \texttt{shap\_comparison\_summary.json} \\
